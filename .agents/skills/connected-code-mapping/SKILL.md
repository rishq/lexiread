...
Follow the shared artifact contract in `.agents/planning/planning_contract.md`.
...
Represent impacted or planned workflows as numbered use cases. Use hierarchical numbering when a parent workflow is refined into narrower validation, compatibility, success, or rejection subcases. Keep workflow lines and optional layers under each use case according to `.agents/planning/planning_contract.md`.
...
- if layered output is requested, preserves mapping-first reasoning while staying syntax-compatible with `.agents/planning/planning_contract.md`
